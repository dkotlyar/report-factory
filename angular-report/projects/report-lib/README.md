...
7. В шаблоне обязательно должны быть указаны идентификаторы контента, вызываемые в цикле ngFor. 
   Идентификатор указывается меткой #content на корневом блоке цикла. 
   Все остальные элементы считаются статическим контентом.
Пример:
    ```angular2html
    <table>
        <tr>
...
Директива устанавливается на HTML-элементы `td` и позволяет развернуть текст
для отображения в вертикальном стиле.
